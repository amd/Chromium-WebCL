...
void build_inter_pred_param_sec_ref_ocl(const int plane,
  ...
  inter_ocl_obj.pred_param_cpu_sec_pre[tile_num]->filter_x_mv =
    filter_radix + subpix->filter_x[subpel_x] - filter;
  inter_ocl_obj.pred_param_cpu_sec_pre[tile_num]->x_step_q4 = xs;
  inter_ocl_obj.pred_param_cpu_sec_pre[tile_num]->filter_y_mv =
...
void build_inter_pred_param_fri_ref_ocl(const int plane,
  ...
  if (/*pred_mode == 6 &&*/ !ref_idx && xs == 16 && ys == 16 &&
      buf_offset > 0 && buf_offset < inter_ocl_obj.buffer_size && !CpuFlag) {
    ...
    pthread_mutex_lock(&inter_ocl_obj.index_mutex);
    case_num = atomic_add(&inter_ocl_obj.atomic_case_num[pred_mode >> 1], h*w) - h*w;
    pthread_mutex_unlock(&inter_ocl_obj.index_mutex);
	//printf("tile_num %d: case_num[%d] = %d\n", tile_num, tile_num,case_num[tile_num]);
    for (j = 0; j < h; ++j) {
      for (i = 0; i < w; ++i) {
        inter_ocl_obj.index_param_num_gpu_pre[tile_num][0] =
            tile_num * inter_ocl_obj.tile_param_count_gpu +
            inter_ocl_obj.gpu_block_count[tile_num];
        inter_ocl_obj.index_xmv_gpu_pre[tile_num][0] =
            (j << 2) * pre_buf->stride + (i << 2) - 3 * sub_y * pre_buf->stride -
            3 * sub_x + buf_offset + src_num * inter_ocl_obj.buffer_size;
        inter_ocl_obj.dst_index_xmv_gpu_pre[tile_num][0] =
            (dst - dst_fri) + (j << 2) * (dst_buf->stride) + (i << 2) +
            (cm->new_fb_idx * inter_ocl_obj.buffer_size);

        inter_ocl_obj.index_param_num_gpu_pre[tile_num]++;
        inter_ocl_obj.index_xmv_gpu_pre[tile_num]++;
        inter_ocl_obj.dst_index_xmv_gpu_pre[tile_num]++;

       // pthread_mutex_lock(&inter_ocl_obj.index_mutex);

        //case_num = atomic_inc(&inter_ocl_obj.atomic_case_num[pred_mode >> 1]) - 1;
        inter_ocl_obj.index_case_gpu[
              case_num  + j * w + i + (pred_mode >> 1) * inter_ocl_obj.param_count_gpu_all] =
                 tile_num * inter_ocl_obj.tile_param_count_gpu +
                 inter_ocl_obj.gpu_index_count[tile_num] + j * w + i;

       //pthread_mutex_unlock(&inter_ocl_obj.index_mutex);
      }
    }
    inter_ocl_obj.gpu_index_count[tile_num] += h * w;
    ...
    inter_ocl_obj.pred_param_cpu_fri_pre[tile_num]->filter_x_mv =
      filter_radix + subpix->filter_x[subpel_x] - filter;
    inter_ocl_obj.pred_param_cpu_fri_pre[tile_num]->x_step_q4 = xs;
    inter_ocl_obj.pred_param_cpu_fri_pre[tile_num]->filter_y_mv =
      filter_radix + subpix->filter_y[subpel_y] - filter;
