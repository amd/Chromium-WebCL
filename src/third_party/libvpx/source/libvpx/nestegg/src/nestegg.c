...
#define E_LAST \
  { NULL, 0, ( enum ebml_type_enum)0, 0, DESC_FLAG_NONE, NULL, 0, 0 }
...
ne_pool_init(void)
{
  ...
  pool = (struct pool_ctx*)h_malloc(sizeof(*pool));
  if (!pool)
    abort();
  return pool;
}
...
ne_read_string(nestegg * ctx, char ** val, uint64_t length)
{
  ...
  if (length == 0 || length > LIMIT_STRING)
    return -1;
  str = (char*)ne_pool_alloc(length + 1, ctx->alloc_pool);
...
ne_read_binary(nestegg * ctx, struct ebml_binary * val, uint64_t length)
{
  if (length == 0 || length > LIMIT_BINARY)
    return -1;
  val->data = (unsigned char*)ne_pool_alloc(length, ctx->alloc_pool);
  val->length = length;
...
ne_ctx_push(nestegg * ctx, struct ebml_element_desc * ancestor, void * data)
{
  ...
  item = (struct list_node*)ne_alloc(sizeof(*item));
...
ne_read_master(nestegg * ctx, struct ebml_element_desc * desc)
{
  ...
  node = (struct ebml_list_node*)ne_pool_alloc(sizeof(*node), ctx->alloc_pool);
  node->id = desc->id;
...
ne_find_track_entry(nestegg * ctx, unsigned int track)
{
  ...
  while (node) {
    ...
    if (track == tracks)
      return (struct track_entry*)node->data;
    tracks += 1;
    node = node->next;
  }
...
ne_read_block(nestegg * ctx, uint64_t block_id, uint64_t block_size, nestegg_packet ** data)
{
  ...
  *data = NULL;
  ...
  cluster = (struct cluster *)(ctx->segment.cluster.tail->data);
  ...
  pkt = (nestegg_packet*)ne_alloc(sizeof(*pkt));
  pkt->track = track - 1;
  ...
  for (i = 0; i < frames; ++i) {
    ...
    f = (struct frame*)ne_alloc(sizeof(*f));
    f->data = (unsigned char*)ne_alloc(frame_sizes[i]);
...
ne_find_seek_for_id(struct ebml_list_node * seek_head, uint64_t id)
{
  ...
  while (seek_head) {
    ...
    head = (struct ebml_list*)seek_head->data;
    seek = head->head;

    while (seek) {
      assert(seek->id == ID_SEEK);
      s = (struct seek*)(seek->data);
...
ne_find_cue_point_for_tstamp(struct ebml_list_node * cue_point, uint64_t scale, uint64_t tstamp)
{
  ...
  while (cue_point) {
    ...
    c = (struct cue_point*)(cue_point->data);
    ...
    prev = (struct cue_point*)(cue_point->data);
...
nestegg_init(nestegg ** context, nestegg_io io, nestegg_log callback)
{
  ...
  ctx = (nestegg*)ne_alloc(sizeof(*ctx));

  ctx->io = (nestegg_io*)ne_alloc(sizeof(*ctx->io));
  *ctx->io = io;
  ctx->log = callback;
...
nestegg_track_seek(nestegg * ctx, unsigned int track, uint64_t tstamp)
{
  ...
  while (node) {
    ...
    pos = (struct cue_track_positions*)node->data;
