...
void *Context::getTextureUltimatePointer(GLuint tex) 
{ 
	return mResourceManager->getTextureUltimatePointer(tex);
}
void *Context::getBufferUltimatePointer(GLuint buf) 
{ 
...
cl_mem Context::clCreateFromGLBuffer (	cl_context context,
 	...
 	GLuint bufobj,
 	cl_int * errcode_ret) {
		void *buf = this->getBufferUltimatePointer(bufobj);
		return mRenderer->clCreateFromGLBuffer(context, flags, buf, errcode_ret);
}

cl_mem Context::clCreateFromGLTexture (	cl_context context,
 	cl_mem_flags flags,
 	GLenum texture_target,
 	GLint miplevel,
 	GLuint texture,
 	cl_int *errcode_ret){
		void *tex = this->getTextureUltimatePointer(texture);
		return mRenderer->clCreateFromGLTexture(context, flags, texture_target, miplevel, tex, errcode_ret);
}

cl_mem Context::clCreateFromGLRenderbuffer (	cl_context context,
 	cl_mem_flags flags,
 	GLuint renderbuffer,
 	cl_int * errcode_ret){
	return NULL;
...
cl_int Context::clEnqueueAcquireGLObjects (	cl_command_queue command_queue,
 	cl_uint num_objects,
 	const cl_mem *mem_objects,
 	cl_uint num_events_in_wait_list,
 	const cl_event *event_wait_list,
 	cl_event *event){
		return mRenderer->clEnqueueAcquireGLObjects(command_queue,
				num_objects,
				mem_objects,
...
cl_int Context::clEnqueueReleaseGLObjects (	cl_command_queue command_queue,
 	cl_uint num_objects,
 	const cl_mem *mem_objects,
 	cl_uint num_events_in_wait_list,
 	const cl_event *event_wait_list,
	...
	return mRenderer->clEnqueueReleaseGLObjects(command_queue, num_objects,
			mem_objects, num_events_in_wait_list,
			event_wait_list, event);
}

cl_int Context::clGetGLObjectInfo (	cl_mem memobj,
 	cl_gl_object_type *gl_object_type,
 	GLuint *gl_object_name) {
		return CL_SUCCESS;
}

cl_int Context::clGetGLTextureInfo (	cl_mem memobj,
